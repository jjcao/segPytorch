...
def make_layers_vgg16sps(cfg, n_classes):
    in_channels = 3
    kernel_size = 3
    padding = 100
    conv_blocks = nn.ModuleList()
...
class FCN32sps(FCN):
    def __init__(self, n_classes=2, learned_billinear=False):
        super(FCN32sps, self).__init__(n_classes, learned_billinear)

        self.learned_billinear = learned_billinear
        self.n_classes = n_classes
        
        self.conv_blocks, self.classifier = make_layers_vgg16sps(cfg['vgg16'], self.n_classes) 
        
        if self.learned_billinear:            
            self.upscore = nn.ConvTranspose2d(self.n_classes, self.n_classes, 32, stride=16, bias=False)
